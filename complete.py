...
def clusterDistCompleteLinkage(cluster1, cluster2, distMat):
    maxDist = -float('inf')
    for i in cluster1:
        for j in cluster2:
...
def distMatUpdateCompleteLinkage(distMat, clusters):
    numClusters = len(clusters)
    for i in range(numClusters):
        for j in range(i + 1, numClusters):
            dist = clusterDistCompleteLinkage(clusters[i], clusters[j], distMat)
            distMat[i][j] = dist
            distMat[j][i] = dist
...
def clusteringCompleteLinkage(labels, distMat):
    ...
    while len(clusters) > 1:
        ...
        distMatUpdateCompleteLinkage(distMat, clusters)
...
def main():
    ...
    clusteringCompleteLinkage(labels, distMat)
